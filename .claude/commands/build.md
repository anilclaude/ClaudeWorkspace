...
Runs the core loop for the next `status: ready` task in `platform/docs/task-ledger-<prd-slug>.md` — each PRD has its own standalone ledger file, never a shared one. Everything builds directly on `master`; if more than one PRD has `ready` tasks, ask which to build rather than guessing (mirroring `/plan`'s multi-PRD prompt). Pass a PRD slug to target one directly (`/build cafe-access-roles`).
