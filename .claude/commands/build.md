...
Runs the core loop for the next `status: ready` task in `platform/docs/taskplanned/task-ledger-<prd-slug>.md` — each PRD has its own standalone ledger file, never a shared one. Everything builds directly on `master`; if more than one PRD has `ready` tasks, ask which to build rather than guessing (mirroring `/plan`'s multi-PRD prompt). Pass a PRD slug to target one directly (`/build cafe-access-roles`). `/plan cr` also invokes this command automatically immediately after planning, passing the CR's slug explicitly (`/build cr-<slug>`) — the multi-ledger prompt above never fires in that path since the target is already named.

1. **builder** works directly on `master` (halting on a dirty tree — no branch to check out), implements the task with tests, and runs `build`, `typecheck`, `lint`, and `test` **scoped to the package(s) touched** (`pnpm --filter <package> ...` — see `builder.md`'s "Gate scope" note; the full four-gate command is a `/commit`-time check, not repeated per task). Sets `status: in_review`. Logs each checkpoint to `platform/docs/build-trace.md` as it happens — `start`, `context_read`, one `file_written` row per file, `tests_written`, `gates_start`, `gates_passed`, `submit_for_review` (see `builder.md`'s "Trace logging" section and that file's header for the row shape).
...
Nothing is committed. When the loop clears, the diff sits uncommitted on `master` — run `/commit` when you're ready.
