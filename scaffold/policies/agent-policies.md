...
**Ledger `note:` discipline** (tightened 2026-08-11, after `task-ledger-cafe-menu-management.md`'s notes drifted to 15-40 lines per task): optional, 1-3 sentences when present — P4/P6 combining justification, or a pointer to a `DECISIONS.md` entry by name. Never implementation detail (schema shapes, field lists, endpoint bodies) — that's the builder's job at build time, not the planner's to pre-write.
