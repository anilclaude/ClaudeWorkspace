...
//
// Service-local for now. When a second service needs this, promote it to
// backend/libs/testing per repo-structure.md's promotion path rather than
// copying it — duplicated DB lifecycle code is exactly what libs/ exists for.
...
function required(name: string): string {
  ...
  if (!value) {
    throw new Error(`Missing ${name} — check .env.test.`);
  }
  return value;
}
...
function build(): DataSource {
  ...
  if (!database.endsWith('_test_db')) {
    throw new Error(
      `Refusing to run integration tests against "${database}". ` +
        'POSTGRES_DB must end in _test_db — check .env.test.',
    );
  }
  ...
  return new DataSource({
    ...
    // the THIRD of three files that must set the same value. The database is
    // shared across every backend service, so a mismatch here wouldn't just
    // be inconsistent, it'd point this test harness at a table another
    // service also thinks is its own migration history. This is the exact
    // bug that motivated this template existing — grep all three files for
    // __SERVICE_NAME___migrations after replacing the placeholder to confirm
    // none were missed.
 ...
 * Throws an actionable message if Postgres isn't reachable — integration
 * tests fail loudly rather than passing vacuously against no database.
 */
