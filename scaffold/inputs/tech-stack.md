...
Stack profile: **right-sized services** — NestJS services (own DB each) + Next.js
BFF web app, custom JWT auth. Shape declared in `repo-structure.md`.
...
| Layer | Choice | Range | Installed |
|---|---|---|---|
| Framework | Next.js (App Router, BFF pattern) | `^16.2.10` | 16.3.0 |
...
| HTTP client | Axios — **server-side only**, inside BFF route handlers | `^1.18.1` | 1.19.0 |
| Icons | lucide-react | `^1.25.0` | 1.28.0 |

Browsers never call the backend directly. All backend traffic goes through this
app's own Route Handlers using server-only environment variables.
...
| 2026-08-06 | Retries on idempotent methods only, in the BFF client | Retrying a POST could double-submit and defeat the in-flight guard the login PRD requires (AC3). |
