...
export const envSchema = z.object({
  ...
  // Shared across every backend service that verifies a login token — must be
  // the exact same value backend/auth signs with (cafe-access-roles T02: no
  // new token/claims format is invented here, only verified). Never logged.
