...
function required(name: string): string {
  ...
  if (!value) {
    throw new Error(`Missing ${name} — check backend/auth/.env.test.`);
  }
  return value;
}
  ...
  if (!database.endsWith('_test_db')) {
    throw new Error(
      `Refusing to run integration tests against "${database}". ` +
        'POSTGRES_DB must end in _test_db — check backend/auth/.env.test.',
    );
  }
 ...
 * Throws a actionable message if Postgres isn't reachable — integration tests
